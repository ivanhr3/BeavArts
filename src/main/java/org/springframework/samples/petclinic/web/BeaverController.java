...
public class BeaverController {
    ...
    @GetMapping("beaverInfo/perfil/edit")
    public String initActualizarPerfil(@RequestParam int beaverId, ModelMap model) {
        ...
        if(user.getUsername().equals(beaver.getUser().getUsername())) {
            ...
            model.addAttribute("perfil", beaver.getPerfil());
    ...
    @PostMapping("beaverInfo/perfil/edit")
    public String processActualizarPerfil(@Valid Perfil perfil, BindingResult result, ModelMap model) {
        if (result.hasErrors()) {
