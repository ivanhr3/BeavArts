...
public class EncargoValidatorTest {
	...
	void shouldNotValidateWhenTituloIsBlank() {
		...
		e.setTitulo("");
		e.setPrecio(50.00);
		e.setDisponibilidad(EncargoStatus.Si);
	...
	void shouldNotValidateWhenPrecioHasMoreThan6DigitsInHisIntegerPart() {
		...
		Assertions.assertThat(c.size()).isEqualTo(1);
		ConstraintViolation<Encargo> violation = c.iterator().next();
		Assertions.assertThat(violation.getPropertyPath().toString()).isEqualTo("precio");
		Assertions.assertThat(violation.getMessage()).isEqualTo("valor numérico fuera de los límites (se esperaba <6 dígitos>.<2 dígitos)");
	}
	...
	void shouldNotValidateWhenPrecioLessThan0() {
		...
		Assertions.assertThat(con.getPropertyPath().toString()).isEqualTo("precio");
		Assertions.assertThat(con.getMessage()).isEqualTo("tiene que ser mayor o igual que 0");
	}

	@Test
	@DisplayName("No debe validarse con el atributo 'descripción' con más de 3000 caracteres")
	void shouldNotValidateWhenDescripcionHasMoreThan3000Characters() {
		...
		//ASSERT
		assertTrue(e.getDescripcion().length() > 3000);
		ConstraintViolation<Encargo> con = c.iterator().next();
		Assertions.assertThat(con.getPropertyPath().toString()).isEqualTo("descripcion");
		Assertions.assertThat(con.getMessage()).isEqualTo("el tamaño tiene que estar entre 30 y 3000");
	}

	@Test
	@DisplayName("No debe validarse con el atributo 'descripción' con menos de 30 caracteres")
